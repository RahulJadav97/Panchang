...
class HoraController extends GetxController{

  var loading  = false.obs;
  var oldResponse = HoraModel().obs;



  Future<void> horaCont () async {
    try{
      loading(true);

      final newResponse = await ApiServices().getHora(currentDay.toString(),currentMonth.toString(),currentYear.toString());
      if(newResponse.responseCode == 1){
        oldResponse = newResponse.obs;
        print("Hora Data Cont Successful");
      }else{
        print("Hora Data Cont failed..");
      }
    }finally{
      loading(false);
    }
